...
### Options Chain Architecture (Schwab-Only)
The options chain is sourced exclusively from Schwab. The `/api/market/options` endpoint fetches chain data from the Schwab REST API (`/marketdata/v1/chains` with `range: NTM`, `strikeCount: 60`), providing the initial snapshot with bid/ask/Greeks/IV/volume/OI. On the frontend, the Schwab WebSocket streamer (`LEVELONE_OPTIONS:SUBS`) provides real-time updates to individual contract cells via the `useOptionsStreamStore`. DataCell components use a streaming-first pattern: `getStreamVal(tick, key) ?? getContractVal(contract, key)`. Polygon is NOT used for the options chain — it is reserved for flow aggregates, data analysis, and AI strategy scanning. The StrategyBuilder also uses this streaming-first pattern: `enrichLeg`/`updateLeg` check the streaming store first, falling back to Schwab REST data. The `spreadPrices` calculation treats missing bid/ask as 0 with a $0.01 minimum to prevent the review button from being grayed out on multi-leg trades.
