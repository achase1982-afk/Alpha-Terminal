...
The equity backfill pipeline uses two Polygon-based approaches (no Schwab token needed): (1) `updateEquityDailyFromGroupedBars()` — the primary startup fast-path that calls Polygon's bulk grouped daily bars endpoint (`/v2/aggs/grouped/locale/us/market/stocks/{date}`) once per trading date, returning all US stocks in one call; used on startup to fill the last 5 trading weekdays for all 130 LC130 symbols with no per-symbol rate limits. (2) `backfillEquityFromPolygon()` — the legacy per-symbol backfill with 13s inter-symbol delays and 15s/30s/45s retry backoffs for Polygon's free tier. The startup trigger in `index.ts` computes the last 5 weekdays dynamically and calls `updateEquityDailyFromGroupedBars`. The `getCompactUniverseSummaries()` function filters `latestDate` to LC130 symbols only (not global max) to avoid returning empty summaries when non-LC130 symbols have more recent data. The analyst validation layer normalizes LLM-generated `timeHorizon` values (e.g., "7-14D" → "10+D") instead of rejecting them.
