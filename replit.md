...
A "Discovery Mode (BETA)" scanner uses a sophisticated scoring engine with DB-backed data for faster scans, focusing on Setup Quality, Accumulation Pattern, IV Setup, Flow Divergence, and Emerging Relative Strength. The liquidity gate in `computeLiqFromStrikes` falls back to a passing score (15) when no strikes exist in the 20-60 DTE range (e.g., historical backfill data with short DTEs only). The daily snapshot cron (`scheduleDailySnapshot`) runs at 21:30 UTC (4:30 PM ET) on trading days, calling `runFullSnapshot` for the LC130 universe. Flow backfill via `POST /api/snapshot/backfill-flow` supports resumable Polygon flow data collection. The Strategist includes a Strike Resolver to convert AI outputs into specific vertical spread trades, integrated with SSE for real-time data streaming.
