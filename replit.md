...
-   **Market News**: Dual-source: Finnhub API (primary, `FINNHUB_API_KEY`) + Yahoo Finance RSS (secondary, no key). Merged, deduplicated, sorted newest-first. Auto-refreshes every 2 minutes. Yahoo Finance RSS uses symbol-specific feeds (`/rss/headline?s=AAPL`) plus general top stories. Futures/index symbols mapped to Yahoo format (e.g. `/ES` → `ES=F`, `$SPX` → `^GSPC`). API server uses `--max-http-header-size=65536` for Yahoo cookie headers.
