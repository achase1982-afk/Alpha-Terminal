...
-   **Account Information**: Bloomberg-style modal with Overview/Positions/Orders/Activity tabs. Proxies Schwab Trader API (`/api/account/summary`, `/api/account/transactions`, `/api/account/orders`). Overview shows account value, buying power, cash, day P&L, equity/margin metrics. Positions tab has interactive table with P&L coloring; clicking equity symbols navigates to that ticker. Orders tab shows status badges (FILLED/WORKING/CANCELED). Activity tab shows 30-day transaction history. Uses `traderAccessToken || accessToken` for auth.
