...
-   **Connection State**: 3-state `streamStatus` (`"offline"` | `"connecting"` | `"live"`) driven by backend `streamerStatus` SSE events. Backend is the single source of truth: broadcasts `connected` on Schwab LOGIN ack, `disconnected` on WS close, `rejected` on LOGIN failure (stops auto-reconnect; requires re-auth). Frontend maps `connected→live`, `disconnected→connecting`, `rejected→offline`.
...
-   **Market News**: Finnhub API integration (`/api/market/news?symbol=X`) providing real-time company news. Dedicated NEWS tab between CHART and OPTIONS. Fetches last 7 days of news, auto-refreshes every 5 minutes. Requires `FINNHUB_API_KEY` env secret. Does not require Schwab auth.
