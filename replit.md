...
Authentication combines Clerk for app-level login and Schwab OAuth 2.0 for Market Data and Accounts & Trading APIs, with a chained single-login flow. Schwab tokens are persisted server-side in `.data/schwab-tokens.json` and auto-refreshed proactively. **Dev/Prod Auth Split**: In development (`NODE_ENV !== "production"` / `import.meta.env.DEV`), Clerk auth is fully bypassed — the backend skips `clerkMiddleware()` and `apiRequireAuth`, and the frontend renders the app directly without the sign-in gate (but still wraps in `ClerkProvider` so hooks like `useClerk()` don't crash). In production, full Clerk auth is enforced with `SignedIn`/`SignedOut` gates and server-side token verification. Market News is sourced from Finnhub and Polygon.io, merged and deduplicated. An in-app article reader uses a server-side proxy to fetch and clean HTML content. Earnings dates are fetched from Yahoo Finance. AI integration uses Gemini models (`gemini-2.5-flash` and `gemini-2.5-pro`) with strict data grounding rules for market briefing, options strategy, and technical analysis. Technical analysis uses the `technicalindicators` npm package. Zustand is used for state management, with React Query for data fetching and error handling. The build system uses `esbuild` and `tsc`.
