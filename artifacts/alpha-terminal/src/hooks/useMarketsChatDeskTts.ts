...
export const MARKETS_CHAT_AUDIO_SKIP_SECONDS = 15;
const TOUCH_MIN = 44;

/**
 * Desk-buffered Edge TTS for Markets chat assistant bubbles (merged MP3, same pipeline as Desk card).
 */
export function useMarketsChatDeskTts(args: {
  symbol: string;
  ...
  const { symbol, containerRef } = args;
  ...
  const startPlay = useCallback(
    async (messageId: string, plainText: string) => {
      ...
      const deskResultId = `markets-chat-${symbol}-${messageId}`;
      const deskAudioChunks = splitDeskAudioTextIntoChunks(trimmed);
      if (deskAudioChunks.length === 0) return;

      const deskTtsWarmKey = `${deskResultId}\0${trimmed}\0${JSON.stringify(VOICE)}`;
      ...
      const ensureSession = async (): Promise<{ sessionId: string; totalChunks: number } | null> => {
        ...
        try {
          const res = await fetchWithAuth("/api/tts/desk-audio/start", {
            ...
            body: JSON.stringify({ text: trimmed, voiceConfig: VOICE }),
            signal: ac.signal,
  ...
  const iconBtnBase: CSSProperties = useMemo(
    () => ({
      minWidth: TOUCH_MIN,
      minHeight: TOUCH_MIN,
      display: "inline-flex",
