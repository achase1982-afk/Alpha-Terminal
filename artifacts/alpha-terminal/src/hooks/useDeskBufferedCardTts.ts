...
/**
 * Shared merged-chunk desk TTS (`/api/tts/desk-audio`) for Strategist cards (Desk, Validation, etc.).
 */
export function useDeskBufferedCardTts(args: {
  plainText: string;
  /** Passed to `emitDeskTtsClientEvent` as `deskResultId` (telemetry / cache partition). */
  audioId: string;
  voiceConfig: Record<string, unknown>;
  sessionRateStorageKey: string;
  ...
  const { plainText, audioId, voiceConfig, sessionRateStorageKey, resetDependency, containerRef } = args;

  const deskAudioChunks = useMemo(() => splitDeskAudioTextIntoChunks(plainText), [plainText]);
  const deskTtsWarmKey = useMemo(
    () => `${audioId}\0${plainText.trim()}\0${JSON.stringify(voiceConfig)}`,
    [audioId, plainText, voiceConfig],
  );

  const [speechRate, setSpeechRateState] = useState<number>(() => {
    if (typeof window === "undefined") return 1;
    const v = Number(sessionStorage.getItem(sessionRateStorageKey));
  ...
  const setSpeechRate = useCallback(
    (r: number) => {
      ...
      try {
        sessionStorage.setItem(sessionRateStorageKey, String(r));
      } catch {
        /* QuotaExceededError */
      }
  ...
  }, [audioId, deskAudioChunks, deskTtsWarmKey, plainText, revokeObjectUrl, voiceConfig]);
  ...
  }, [audioId, deskAudioChunks, deskTtsWarmKey, plainText, voiceConfig]);
