...
export function TradingChart({ data }: TradingChartProps) {
  ...
  useEffect(() => {
    ...
    const formattedData = sortedData.map(c => {
      // Lightweight charts expects time in UNIX timestamp (seconds) or 'YYYY-MM-DD'
      const time = (new Date(c.datetime).getTime() / 1000) as Time;
      return {
        time,
        open: c.open,
        high: c.high,
    ...
    if (overlays.sma20) {
      ...
      const sma20Series = chart.addLineSeries({ color: '#58A6FF', lineWidth: 2, title: 'SMA 20' });
    ...
    if (overlays.sma50) {
      const sma50Data = calculateSMA(sortedData, 50)
        .filter(d => d.value !== null)
        .map(d => ({ time: (new Date(d.time).getTime() / 1000) as Time, value: d.value as number }));
      const sma50Series = chart.addLineSeries({ color: '#D2A8FF', lineWidth: 2, title: 'SMA 50' });
  ...
  return (
    <div className="w-full h-[500px] relative rounded-xl border border-card-border bg-[#0D1117] overflow-hidden shadow-inner">
