...
interface Props {
  children: ReactNode;
  /** Shown in the crash UI, e.g. "Strategist tab" or "Scanner" */
  tabName: string;
}
...
/**
 * Catches render errors in a bottom-tab subtree (Strategist, Scanner, etc.)
 * so a thrown error does not blank the entire panel on mobile.
 */
      ...
      return (
        <div className="max-w-4xl mx-auto p-6 rounded-xl border border-red-500/40 bg-red-950/40 space-y-4">
          <p className="text-sm font-bold text-red-300 uppercase tracking-wide">
            {this.props.tabName} crashed: <span className="font-mono normal-case text-red-200/95">{msg}</span>
          </p>
          <button
            type="button"
