...
import { chatLongWaitHint } from "./chatModelUi";
...
type Props = {
  ...
  /** Active chat model id (single-model or synthesizer turns). */
  modelId?: string;
  /** Parallel multi-agent model ids — long-wait copy lists all of them. */
      ...
      <div className="min-w-0 flex flex-col gap-1">
        ...
        {longWait ? (
          <span className="font-mono text-[10px] text-white/40 leading-snug">
            {chatLongWaitHint({ elapsedSec, modelId, multiAgentModels: multiAgent ? multiAgentModels : undefined })}
          </span>
        ) : null}
      </div>
    </div>
