...
/** v5 appends Claude Sonnet 4.6 + Haiku 4.5 (extended thinking); indices 0–6 unchanged. */
async function migrateStrategistModelCatalogV4ToV5IfNeeded(merged: StrategistConfig): Promise<boolean> {
  if (merged.strategistModelCatalogVersion !== 4) {
    return false;
  }
  const now = new Date();
  await db
...
export async function getSettings(): Promise<StrategistConfig> {
  ...
  settingsLoadInFlight = (async (): Promise<StrategistConfig> => {
    ...
    try {
      ...
      if (totalMs > 200 || migMs > 0 || bumpMs > 0 || bumpV4Ms > 0 || bumpV5Ms > 0) {
        logger.info(
          {
...
export function getSettingMeta(): SettingMetaEntry[] {
  ...
  return [
    ...
    { key: "strategistConvictionModelIdx", label: "Conviction Desk", group: "Strategist", default: 0, min: 0, max: STRATEGIST_MODEL_OPTIONS.length - 1, step: 1, description: "Single-pass trade memo JSON for Conviction Desk. Catalog indices: 0 = Claude Opus 4.7, 1 = GPT-5.5 + Thinking, 2 = GPT-5.4 Mini, 3 = Gemini 3 Pro (JSON MIME; catalyst web search is pre-run when needed), 4 = Gemini 3 Flash, 5 = Grok, 6 = Claude Sonnet 4.6 + thinking, 7 = Claude Haiku 4.5 + thinking.", options: modelOptions },
