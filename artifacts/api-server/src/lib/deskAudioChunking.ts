/**
 * Shared Desk TTS chunk splitting: ~2.5s audio target, sentence-first, clause fallback, no mid-word breaks.
 * Used by the API server buffer and mirrored in the alpha-terminal client for consistent indices.
 */

/** Roughly 40–60 words ≈ 2.5s TTS; cap well under OpenAI 4096 char limit. */
...
/** Last index in [0, maxIdx] that is not inside a word (boundary after maxIdx is ok). */
...
/**
 * Split plain text into TTS chunks. Preserves caller trimming expectations: empty input → [].
 */
