...
**Session-date attribution (intraday.vwap / intraday.rsi / intraday.equity_block_tape):** Each block may include **vwap_as_of_session_date**, **rsi_as_of_session_date**, and **equity_block_tape_as_of_session_date** (ISO **YYYY-MM-DD**, US equity **session** date in America/New_York). When these match **today's** session date, the metrics are anchored to the **current** regular session when tape or minute data exists. When they show an **earlier** calendar date, the **current** session had **no usable data** for that metric and the value was **filled from the most recent prior session** (or, for RSI, from the **latest minute closes** spanning sessions). Treat earlier dates as **stale relative to a live intraday tape**—down-weight conviction from those fields versus fresh session activity. Order book and streamed equity quotes do not use this fallback and may remain null when stale.
`;
