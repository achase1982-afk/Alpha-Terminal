...
- **sessionTape** when present. Check **sessionTape.tapeKind**: "live" — **execPerStrike** (sweep/block/regular counts and notionals per strike for the session), **topPrints** (largest session prints with sweep/block flags, **side** ask/bid/mid when available, and **openClose** open/close/unknown from Polygon trade conditions when classifiable), **aggressorByStrike**, **openCloseByStrike** (per-strike open vs close vs unknown print counts), **aggressorSessionTotals**. "eod_fallback" — same object shape but synthesized from EOD volume only: **sweepCount**/**blockCount** are zero, **side** is null, **openClose** on prints is null, **openCloseByStrike** is empty, aggressor mix is unknown; use it to rank where volume concentrated, not for sweep/block or buyer/seller lean.
...
When **sessionTape.tapeKind** is "live", anchor **dominant_flow**, **institutional_signal**, **retail_signal**, **key_strikes**, and **read** in concrete tape facts: sweeps, blocks, ask/bid lean, largest prints, and **openClose** / **openCloseByStrike** when populated (often unknown until Polygon exposes explicit open/close condition semantics). When "eod_fallback", describe flow from volume concentration and unusual vol/OI only; do not claim sweeps, blocks, aggressor, or open/close. If **sessionTape** is null, lean on the EOD per-strike snapshot and chain unusual activity only. Do not invent sweep counts.
...
- **polygonFlowHighlights.sessionTape** (**tapeKind**, **tapeBackfillReason**, sweeps, blocks, aggressor totals, **openCloseByStrike**, top prints with **side** and **openClose**).
